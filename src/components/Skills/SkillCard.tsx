  ...
  return (
    <div className={`relative inline-block ${className}`}>
      <div
        ...
        style={{
          filter: `saturate(${highlight ? 1 : 0.5}) drop-shadow(0 0 ${highlight ? "15px" : "0px"} ${color})`,
        }}
      >
        <img
          src={skill.hero}
          ...
          style={{
            filter: highlight ? "blur(0px)" : "blur(1px)",
            willChange: "filter",
          }}
        />
      ...
      <div
        className={` absolute top-full left-1/2 font-jetbrains transform -translate-x-1/2 mt-2 p-2 bg-white dark:bg-zinc-800 text-yellow-50 dark:text-yellow-50 text-sm rounded shadow-lg transition-all duration-300 ease-out
          ${highlight ? "opacity-100 translate-y-5" : "opacity-0 -translate-y-10"}`}
      >
        {skill.name}
      </div>
    </div>
